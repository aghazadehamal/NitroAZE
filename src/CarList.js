...
const CarList = () => {
  ...
  const toggleLike = (id) => {
    ...
    const handleFilterChange = (e) => {
      ...
      // Fiyat değeri ise, dolar işaretini ve diğer olmayan sayısal karakterleri kaldır
      const cleanedValue = name.includes('Fiyat') ? value.replace(/[^0-9.-]+/g, "") : value;
  ...
  const filteredCars = cars.filter(car => {
    const carPrice = parseFloat(car.price.replace(/[^0-9.-]+/g, "")); // Dolar işareti ve diğer olası para birimi sembollerini kaldır
    return (!filters.marka || car.marka === filters.marka) &&
  ...
  <select className="select-field" name="marka" onChange={handleFilterChange}>
    <option value="">Tüm Markalar</option>
    {/* Marka seçenekleriniz burada olacak */}
  </select>

  <select className="select-field" name="model" onChange={handleFilterChange}>
    <option value="">Tüm Modeller</option>
    {/* Model seçenekleriniz burada olacak */}
  </select>
